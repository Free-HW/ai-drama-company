...
async function x2cCall(payload) {
  reloadEnv();
  const apiKey = await resolveServiceApiKey('x2c');
...
async function resolveServiceApiKey(provider) {
  const normalizedProvider = String(provider || '').trim().toLowerCase();
  if (normalizedProvider === 'giggle' && process.env.GIGGLE_API_KEY) {
    return process.env.GIGGLE_API_KEY;
  }
  if (normalizedProvider === 'x2c' && process.env.X2C_API_KEY) {
    return process.env.X2C_API_KEY;
  }

  const stored = await getLatestServiceCredential(db, normalizedProvider);
  if (stored?.api_key) {
    if (normalizedProvider === 'giggle') {
      process.env.GIGGLE_API_KEY = stored.api_key;
    }
    if (normalizedProvider === 'x2c') {
      process.env.X2C_API_KEY = stored.api_key;
    }
...
async function createGiggleClient() {
  const apiKey = await resolveServiceApiKey('giggle');
  return new GiggleClient({
...
async function ensureX2CApiKeyLoaded() {
  const apiKey = await resolveServiceApiKey('x2c');
  if (!apiKey) {
    throw new Error('X2C_API_KEY 未配置，请先迁移用户的 X2C 凭证');
  }
  process.env.X2C_API_KEY = apiKey;
  return apiKey;
}
...
function buildVisualPrompt(roleLabel, persona) {
  return [roleLabel, persona].filter(Boolean).join(' | ');
}

function normalizeImportedEpisodeStatus(episode) {
  if (episode?.exportUrl) return 'completed';
  if (episode?.giggleProjectId) return 'phase1_done';
  if (episode?.status === 'failed') return 'failed';
  return 'planned';
}

function normalizeImportedProjectStatus(episodes) {
  const hasCompleted = episodes.some((episode) => Boolean(episode.exportUrl));
  if (hasCompleted) return 'completed';
  const hasPhase1 = episodes.some((episode) => Boolean(episode.giggleProjectId));
  if (hasPhase1) return 'ready_for_phase2';
  return 'planned';
}
...
app.post('/api/agent/projects/:projectUuid/episodes/:episodeNo/run', async (req, res) => {
  ...
  try {
    const missing = [];
    if (!(await resolveServiceApiKey('giggle'))) missing.push('GIGGLE_API_KEY');
...
async function runAutoRun(projectUuid) {
    ...
    if (!(await resolveServiceApiKey('giggle'))) {
...
async function runPhase2Only(projectUuid) {
    const project = await getStoryProject(db, projectUuid);
    if (!project) throw new Error('project not found: ' + projectUuid);
    ...
    const episodes = await listProjectEpisodesByUuid(db, projectUuid);
    const pendingEpisodes = episodes.filter((ep) => ep.giggle_project_id && !(ep.status === 'completed' && ep.export_url));
    if (!pendingEpisodes.length) throw new Error('no phase2-ready episodes found');

    const pipelineRunId = require('crypto').randomUUID();
    await createRun(db, {
      runId: pipelineRunId,
      idea: project.idea,
      projectName: `${project.name} 手动启动视频制作`,
    });
    await setProjectId(db, { runId: pipelineRunId, projectId: projectUuid });
    const pipelineEmit = (tagClass, tagText, payload, stage) => {
      addLog(db, { runId: pipelineRunId, stage, tagClass, tagText, payload }).catch(() => {});
    ...
    await setStoryProjectStatus(db, { projectUuid, status: 'running' });
    pipelineEmit('system', 'SYSTEM', `[Phase2] 开始处理 ${pendingEpisodes.length} 集视频制作`, 'system');
    ...
    for (const ep of pendingEpisodes) {
      if (ep.status === 'completed' && ep.export_url) {
        pipelineEmit('system', 'SYSTEM', `[Phase2] EP${ep.episode_no} 已完成，跳过`, 'system');
        continue;
      }
      ...
      try {
        pipelineEmit('system', 'SYSTEM', `[Phase2] EP${ep.episode_no} 开始续跑视频制作`, 'system');
        const retryResult = await agent.runRetry({
          ...
          runId,
          episodeNo: ep.episode_no,
          giggleProjectId: ep.giggle_project_id,
        }, emit);

        const exportUrl = retryResult.export?.videoDownloadUrl || retryResult.export?.videoSignedUrl || '';
        const coverUrl = retryResult.export?.videoThumbnailUrl || deriveCoverUrlFromExportUrl(exportUrl) || ep.cover_url || '';
        ...
        pipelineEmit('system', 'SYSTEM', `[Phase2] EP${ep.episode_no} 完成，可进入发布阶段`, 'system');
      } catch (e) {
        await finishRun(db, { runId, status: 'failed', exportUrl: '' });
        await updateProjectEpisode(db, {
          projectUuid,
        ...
        pipelineEmit('system', 'SYSTEM', `[Phase2] EP${ep.episode_no} 失败: ${e.message}`, 'system');
      }
    }

    const finalEpisodes = await listProjectEpisodesByUuid(db, projectUuid);
    const hasFailed = finalEpisodes.some((ep) => ep.status === 'failed' || ep.status === 'partial_failed');
    const hasPending = finalEpisodes.some((ep) => ep.giggle_project_id && !(ep.status === 'completed' && ep.export_url));

    await setStoryProjectStatus(db, {
      projectUuid,
...
app.post('/api/agent/projects/:projectUuid/start-phase2', async (req, res) => {
  const { projectUuid } = req.params;
  try {
    ...
    runPhase2Only(projectUuid)
      .catch((e) => console.error('[Phase2Only] failed:', e.message))
